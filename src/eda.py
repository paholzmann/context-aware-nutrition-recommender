...
class EDA:
    ...
    def data_type_report(self, df: pd.DataFrame, filepath: str) -> list:
        data_types = df.dtypes
        data_types_count = data_types.value_counts()
        data_types_count.plot(kind="bar")
        plt.title("Data types report")
        plt.xlabel("Columns")
    ...
    def cooking_time_distribution(self, df: pd.DataFrame, filepath: str) -> None:
        cooking_time = df["minutes"].value_counts()
        cooking_time = cooking_time.sort_values()
        cdf = cooking_time.cumsum() / cooking_time.sum()
        plt.figure(figsize=(8, 5))
        plt.plot(cooking_time.values, cdf.values)
        plt.xscale("log")
